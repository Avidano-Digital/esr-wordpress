...
function give_pdf_template_lines( $give_pdf, $give_pdf_payment, $give_pdf_payment_meta, $give_pdf_buyer_info, $give_pdf_payment_gateway, $give_pdf_payment_method, $address_line_2_line_height, $company_name, $give_pdf_payment_date, $give_pdf_payment_status ) {
	...
	$give_pdf->Cell( 33, 6, __( 'Receipt ID', 'give-pdf-receipts' ), 0, 0, 'L', false );
	$give_pdf->SetFont( $font, '', 10 );
	$give_pdf->Cell( 0, 6, give_pdf_get_payment_number( $give_pdf_payment->ID ), 0, 2, 'L', false );
	$give_pdf->SetX( 35 );
	$give_pdf->SetFont( $fontb, '', 10 );
	$give_pdf->Cell( 33, 6, __( 'Transaction Key', 'give-pdf-receipts' ), 0, 0, 'L', false );
	$give_pdf->SetFont( $font, '', 10 );
	$give_pdf->Cell( 0, 6, $give_pdf_payment_meta['key'], 0, 2, 'L', false );
	$give_pdf->SetX( 35 );
	$give_pdf->SetFont( $fontb, '', 10 );
	$give_pdf->Cell( 33, 6, __( 'Donation Status', 'give-pdf-receipts' ), 0, 0, 'L', false );
	$give_pdf->SetFont( $font, '', 10 );
	$give_pdf->Cell( 0, 6, $give_pdf_payment_status, 0, 2, 'L', false );
	$give_pdf->SetX( 35 );
	$give_pdf->SetFont( $fontb, '', 10 );
	$give_pdf->Cell( 33, 6, __( 'Donation Method', 'give-pdf-receipts' ), 0, 0, 'L', false );
	$give_pdf->SetFont( $font, '', 10 );
	...
	$give_pdf->Cell( 122, 6, __( 'Donation Name', 'give-pdf-receipts' ), 0, 0, 'L', false );
	$give_pdf->Cell( 45, 6, __( 'Donation Total', 'give-pdf-receipts' ), 0, 2, 'R', false );
	...
	if ( $give_pdf_payment_meta ) {
		...
		$give_pdf->SetX( 35 );
		$give_pdf->Cell( 0, 11, __( 'Total Donation', 'give-pdf-receipts' ) . ' - ' . html_entity_decode( give_currency_filter( give_format_amount( give_get_payment_amount( $give_pdf_payment->ID ) ) ), ENT_COMPAT, 'UTF-8' ), 0, 2, 'L', false );
