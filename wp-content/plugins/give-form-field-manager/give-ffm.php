...
final class Give_Form_Fields_Manager {
	...
	private function define_globals() {
		...
		if ( ! defined( 'GIVE_FFM_VERSION' ) ) {
			define( 'GIVE_FFM_VERSION', '1.1.3' );
		}

		// Plugin Root File.
		if ( ! defined( 'GIVE_FFM_PLUGIN_FILE' ) ) {
			define( 'GIVE_FFM_PLUGIN_FILE', $this->file );
		}

		// Plugin Folder Path.
		if ( ! defined( 'GIVE_FFM_PLUGIN_DIR' ) ) {
			define( 'GIVE_FFM_PLUGIN_DIR', WP_PLUGIN_DIR . '/' . basename( dirname( $this->file ) ) . '/' );
		}
		...
		if ( ! defined( 'GIVE_FFM_BASENAME' ) ) {
			define( 'GIVE_FFM_BASENAME', plugin_basename( $this->file ) );
		}

		if ( class_exists( 'Give_License' ) ) {
			new Give_License( $this->file, GIVE_FFM_PRODUCT_NAME, GIVE_FFM_VERSION, 'WordImpress' );
		}
	}
