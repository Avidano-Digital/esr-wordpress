...
class Give_FFM_Render_Form {
	...
	function render_items( $form_vars, $post_id, $type = 'post', $form_id, $form_settings ) {
		...
		foreach ( $form_vars as $key => $form_field ) {
			...
			switch ( $form_field['input_type'] ) {
				...
				case 'phone':
					$this->phone( $form_field, $post_id, $type );
					break;
			}
	...
	function text( $attr, $post_id, $type = 'post' ) {
		...
		<input class="textfield<?php echo esc_attr( $this->required_class( $attr ) ); ?>"
		       id="ffm-<?php echo esc_attr( $attr['name'] ); ?>" type="text"
	...
	function textarea( $attr, $post_id, $type ) {
		...
		if ( $rich == 'yes' ) {
			...
			<textarea class="textareafield<?php echo $this->required_class( $attr ); ?>"
			          id="ffm-<?php echo $attr['name']; ?>" name="<?php echo $attr['name']; ?>"
	...
	function select( $attr, $multiselect = false, $post_id, $type ) {
		...
		<select<?php echo $css; ?> id="ffm-<?php echo $attr['name']; ?>"
	...
	function radio( $attr, $post_id, $type ) {
		...
		if ( $attr['options'] && count( $attr['options'] ) > 0 ) {
			foreach ( $attr['options'] as $option ) {
				...
				<label>
					<input name="<?php echo $attr['name']; ?>" type="radio"
	...
	function url( $attr, $post_id, $type ) {
		...
		<input id="ffm-<?php echo $attr['name']; ?>" type="url" class="give-url"
	...
	function email( $attr, $post_id, $type = 'post' ) {
		...
		<input id="ffm-<?php echo $attr['name']; ?>" type="email" class="give_email"
			...
			<table class="give-repeater-table">
				...
				if ( $items ) {
					foreach ( $items as $item ) {
						...
						<tr>
							<td>
								<input id="ffm-<?php echo $attr['name']; ?>" type="text"
								       data-required="<?php echo $attr['required'] ?>"
								       data-type="text"<?php $this->required_html5( $attr ); ?>
								       name="<?php echo esc_attr( $attr['name'] ); ?>[]"
	...
	function date( $attr, $post_id, $type ) {
		...
		<input id="ffm-<?php echo $attr['name']; ?>" type="text"
		       class="datepicker give-ffm-datepicker-<?php echo $attr['name']; ?>"
		       data-required="<?php echo $attr['required'] ?>" data-type="text"<?php $this->required_html5( $attr ); ?>
		...
		<script type="text/javascript">
			jQuery(function ($) {
				...
				$(".give-ffm-datepicker-<?php echo $attr['name']; ?>").datetimepicker({
					dateFormat: '<?php echo $attr['format']; ?>',
					timeFormat: '<?php echo isset( $attr['format_time'] ) ? $attr['format_time'] : 'HH:mm'; ?>',
					hour: give_ffm_hours,
				...
				$(".give-ffm-datepicker-<?php echo $attr['name']; ?>").datepicker({
					dateFormat: '<?php echo $attr['format']; ?>'
				});
	 ...
	 * @param string $type
	 */
	function phone( $attr, $post_id, $type = 'post' ) {
		...
		<input
			...
			id="ffm-<?php echo esc_attr( $attr['name'] ); ?>" type="tel"
			data-required="<?php echo esc_attr( $attr['required'] ); ?>"
